...
export default function CafeDetailScreen({ route, navigation }) {
	...
	const cafeFeatures = currCafe.features.map((currFeatures) => (
		...
		<View
			key={currFeatures.key}
			style={{
				flexDirection: "row",
				backgroundColor: "#d5c1b7",
				marginRight: 10,
				marginBottom: 30,
				padding: 8,
				borderRadius: 8,
		...
		>
			<Icon
				...
				marginRight={5}
			/>
			<Text style={{ color: "#453B3B", fontSize: 18 }}>
				{currFeatures.value}
			</Text>
		</View>
	));
		...
		<View style={styles.container}>
			<ScrollView>
				...
				<Image
					...
					source={{ uri: currCafe.cover_img }}
				/>

				<View style={styles.detailsContainer}>
					<View style={styles.tagContainer}>{cafeTags}</View>
